...
describe('AuthController (e2e)', () => {
  ...
  describe('/login (POST)', () => {
    it('should return 2 errors (phoneNumber should not be empty and phoneNumber must be valid) ', async () => {
      ...
      const response = await request(app.getHttpServer()).post('/auth/login').send(requestBody).expect(HttpStatus.BAD_REQUEST);
      expect(response.body.message).toHaveLength(2);
      expect(response.body.message[0]).toBe('phoneNumber must be a valid phone number');
    ...
    it('should return success response', async () => {
      ...
      const response = await request(app.getHttpServer()).post('/auth/login').send(requestBody).expect(HttpStatus.OK);
